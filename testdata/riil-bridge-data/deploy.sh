...
IMAGE_TAG=dev
TEMPLATE_TAG=dev
VOLUMES=(/data/riil-bridge-data)
IMAGE=""
IMAGES=(registry.cn-beijing.aliyuncs.com\\/ruijie-ruizhi\\/riil-bridge-data)
...
if [ "$IS_DOWN" == "true" ]; then
 ...
 success $"---- riil-bridge-data down successfully.----"
 exit 0
fi
...
if [ -n "docker-compose -f $DOCKER_COMPOSE_FILE ps -q"  ]; then
        note "stopping existing riil-bridge-data instance ..." 
        docker-compose -f $DOCKER_COMPOSE_FILE down -v --remove-orphans
fi
echo ""

h2 "[Step $item]: starting run riil-bridge-data ..."
let item+=1
info "login ${REGISTRY_PREFIX} ...."
docker login ${REGISTRY_PREFIX} --username=${REGISTRY_USERNAME}  --password=${REGISTRY_PASSWORD}

docker-compose --project-name riil-bridge-data -f $DOCKER_COMPOSE_FILE up -d --remove-orphans --pull always || docker-compose --project-name riil-bridge-data -f $DOCKER_COMPOSE_FILE up -d --remove-orphans
echo ""
...
success $"---- riil-bridge-data started successfully.----"
