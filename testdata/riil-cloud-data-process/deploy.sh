...
IMAGE_TAG=dev
TEMPLATE_TAG=dev
VOLUMES=(/data/riil-data-process)
IMAGE=""
IMAGES=(registry.cn-beijing.aliyuncs.com\\/ruijie-ruizhi\\/riil-cloud-data-process)
...
if [ "$IS_DOWN" == "true" ]; then
  ...
  success $"---- riil-cloud-data-process down successfully.----"
  exit 0
fi

h2 "[Step $item]: try create volumes ..."
let item+=1

for volume in ${VOLUMES[@]} 
do
 if [ ! -d $volume ]; then
    mkdir -p ${volume} && chmod -R 777 ${volume}
else
    info " $volume volume already existed skipping ...."
fi
done
echo ""
...
if [ -n "docker-compose -f $DOCKER_COMPOSE_FILE ps -q"  ]; then
        note "stopping existing riil-cloud-data-process instance ..." 
        docker-compose -f $DOCKER_COMPOSE_FILE down -v --remove-orphans
fi
echo ""

h2 "[Step $item]: starting run riil-cloud-data-process ..."
let item+=1
info "login ${REGISTRY_PREFIX} ...."
docker login ${REGISTRY_PREFIX} --username=${REGISTRY_USERNAME}  --password=${REGISTRY_PASSWORD}

docker-compose --project-name riil-cloud-data-process -f $DOCKER_COMPOSE_FILE up -d --remove-orphans --pull always || docker-compose --project-name riil-cloud-data-process -f $DOCKER_COMPOSE_FILE up -d --remove-orphans
...
success $"---- riil-cloud-data-process started successfully.----"
